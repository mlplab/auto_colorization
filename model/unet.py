...
class UNet(torch.nn.Module):
    ...
    def __init__(self, input_ch, output_ch, mode='RGB'):
        ...
        for i in range(1, len(encode_feature) - 1):
            encode_block.append(CNN_Block_for_UNet(encode_feature[i], encode_feature[i + 1], kernel=3, num_layer=2))
        ...
        for i in range(len(decode_feature) - 1):
            up_block.append(torch.nn.ConvTranspose2d(decode_feature[i], decode_feature[i + 1], kernel_size=2, stride=2))
