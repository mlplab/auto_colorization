...
class Dense_UNet(torch.nn.Module):
    ...
    def __init__(self, input_ch, output_ch, first_feature, k, mode='RGB'):
        ...
        self.input_conv = Conv_Block(1, first_feature, 3, 1, 1)
        ...
        for i in range(len(decode_k)):
            up_conv.append(D_Conv_Block(decode_feature[i], decode_feature[i + 1]))
            ...
            pointwise.append(Conv_Block(decode_feature[i + 1] * 2, decode_feature[i + 2], 1, 1, 0))
...
if __name__ == '__main__':
    ...
    model = Dense_UNet(1, 3, 64, [4, 8, 16, 32, 64])
